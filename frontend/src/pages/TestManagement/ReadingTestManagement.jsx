...
const ReadingTestManagement = ({ testCollection, setSidebarData, collectionSkills, setCollectionSkills }) => {
  const { url, token } = useContext(StoreContext)

  const [skillId, setSkillId] = useState(testCollection ? (collectionSkills.find(s => s?.type?.toLowerCase().includes('reading'))?._id) : null)
  ...
  useEffect(() => {
    setSkillId(testCollection ? (collectionSkills.find(s => s?.type?.toLowerCase().includes('reading'))?._id) : null)
  }, [collectionSkills])
  ...
  const handleSaveScript = () => {
    ...
    // persist the title entered by the user
    passage.title = scriptTitle || ''
  ...
  const createTest = async () => {
    ...
    const payload = { testCollectionId: testCollection._id, type: 'reading', title, description, duration: Number(duration), parts }
    try {
    ...
    <div className="ltm_listening_manage">
              ...
              <div className="ltm_part_header">
                ...
                <div className="ltm_part_header_right" style={{ display: 'flex', gap: "12px", alignItems: 'center' }}>
                  <div
                    ...
                    onClick={() => {
                      // open passage popup for this part (do not change selectedPartIndex)
                      ...
                      // prefill passage title if available
                      setScriptTitle(partPassage ? (partPassage.title || '') : '')
      ...
      <div className={`ltm_script_popup ${showScriptPopup ? 'ltm_open' : ''}`}>
        <div className="ltm_script_popup_header">
          <h4>Passage ({scriptContentCount} items)</h4>
          <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
            {!scriptCollapsed && (
        ...
        <div className="ltm_script_popup_body">
          {!scriptCollapsed ? (
            <div>
              <input className="ltm_input" placeholder="Passage title (optional)" value={scriptTitle} onChange={e => setScriptTitle(e.target.value)} style={{ marginBottom: 8 }} />
              ...
              <div className="ltm_script_actions">
                <button className="ltm_btn" onClick={() => { setScriptText(''); setScriptTitle('') }}>Clear</button>
              </div>
            </div>
          ...
          {(() => {
            ...
            return (
              <div style={{ width: '380px',  height: '100%', display: 'flex', flexDirection: 'column' , borderLeft: '1px solid #eee',}}>
                <div className="ltm_script_popup_header" >
                  <h4>Passage ({partPassage.content.length} lines)</h4>
...
export default ReadingTestManagement
